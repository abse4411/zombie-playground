...
const ROGUE_PERKS = [
  { id: 'r_atk',    name: '火力全开', icon: '💢', max: 16, desc: '所有武器伤害 +1%',  apply: p => p.rogueAtk = (p.rogueAtk || 1) + 0.01 },
  { id: 'r_rof',    name: '极速扳机', icon: '⚡', max: 12, desc: '射速 +1%',         apply: p => p.rogueRof = (p.rogueRof || 1) + 0.01 },
  { id: 'r_spd',    name: '疾风步',   icon: '🏃', max: 10, desc: '移动速度 +1%',      apply: p => p.rogueSpd = (p.rogueSpd || 1) + 0.01 },
  { id: 'r_hp',     name: '铁壁',     icon: '❤', max: 16, desc: '最大生命 +3 并回满', apply: p => { p.maxHp += 3; p.hp = p.maxHp; } },
  { id: 'r_mag',    name: '磁力核心', icon: '🧲', max: 8, desc: '宝石磁吸范围 +0.5m', apply: p => p.xpMagnet = (p.xpMagnet || 0) + 0.5 },
  { id: 'r_rel',    name: '快手',     icon: '🧤', max: 10, desc: '换弹速度 +1%',      apply: p => p.rogueRel = (p.rogueRel || 1) - 0.01 },
  { id: 'r_crit',   name: '弱点洞察', icon: '🎯', max: 10, desc: '暴击率 +1%（2倍伤害）', apply: p => p.critChance = (p.critChance || 0) + 0.01 },
  { id: 'r_cash',   name: '贪婪',     icon: '💰', max: 12, desc: '金钱获取 +1%',     apply: p => p.cashMult = (p.cashMult || 1) + 0.01 },
  /* ---- v20.4 扩展池：投掷物 / 支援 / 击退 / 连杀 / 续航（同样小步 1~2 点/级） ---- */
  { id: 'r_thdmg',  name: '破片装药', icon: '🧨', max: 16, desc: '投掷物伤害 +1.5%',   apply: p => p.throwDmg = (p.throwDmg || 1) + 0.015 },
  { id: 'r_thrad',  name: '冲击扩容', icon: '📣', max: 12, desc: '投掷物范围 +2%',   apply: p => p.throwRad = (p.throwRad || 1) + 0.02 },
  { id: 'r_thdur',  name: '延时烈焰', icon: '⏳', max: 12, desc: '投掷物持续效果 +3%', apply: p => p.throwDur = (p.throwDur || 1) + 0.03 },
  { id: 'r_sup',    name: '火力协调', icon: '🛰', max: 12, desc: '支援伤害/范围/持续 +1%', apply: p => { p.supDmg = (p.supDmg || 1) + 0.01; p.supRad = (p.supRad || 1) + 0.01; p.supDur = (p.supDur || 1) + 0.01; } },
  { id: 'r_knb',    name: '蛮力冲击', icon: '🦾', max: 12, desc: '爆炸与近战击退 +3%', apply: p => p.knbMult = (p.knbMult || 1) + 0.03 },
  { id: 'r_streak', name: '连杀狂潮', icon: '🔥', max: 10, desc: '连杀窗口 +0.15s、奖金上限 +0.1', apply: p => { p.streakWinBonus = (p.streakWinBonus || 0) + 0.15; p.streakCapBonus = (p.streakCapBonus || 0) + 0.1; } },
  { id: 'r_regen',  name: '战地急救', icon: '🩹', max: 10, desc: '每秒回复 0.3 生命', apply: p => p.hpRegen = (p.hpRegen || 0) + 0.3 },
  { id: 'r_xp',     name: '快速学习', icon: '📘', max: 10, desc: '经验获取 +2%',     apply: p => p.xpMult = (p.xpMult || 1) + 0.02 },
  { id: 'r_res',    name: '深弹链',   icon: '🎒', max: 10, desc: '备用弹药上限 +5%', apply: p => p.reserveMult = (p.reserveMult || 1) + 0.05 },
  { id: 'r_stamax', name: '耐力强化', icon: '🔋', max: 12, desc: '体力上限 +4',      apply: p => { p.maxStamina += 4; p.stamina += 4; } },
  { id: 'r_stareg', name: '有氧体能', icon: '🫁', max: 10, desc: '体力回复速度 +5%', apply: p => p.staminaRegenMult = (p.staminaRegenMult || 1) + 0.05 },
  { id: 'r_stalight', name: '轻装疾行', icon: '🎽', max: 12, desc: '动作体力消耗 -1.5%', apply: p => p.staminaCostMult = (p.staminaCostMult || 1) - 0.015 },
  { id: 'r_guard',  name: '铁肩硬背', icon: '🛡️', max: 10, desc: '受到的伤害 -1.5%',   apply: p => p.dmgTakenMult = (p.dmgTakenMult || 1) - 0.015 },
  { id: 'r_adr',    name: '肾上腺代谢', icon: '💉', max: 8, desc: '肾上腺素持续 +7.5%', apply: p => p.adrenalineDurMult = (p.adrenalineDurMult || 1) + 0.075 },
];
